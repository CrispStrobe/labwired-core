...
"#;
    ...
    fn every_spelling_of_a_write_action_deserializes_to_one_value() {
        ...
        // must be ALIASES of one value, never separate variants, or a
        // descriptor that says `one_to_clear` where the engine matches
        // `write_one_to_clear` silently does nothing.
        #[derive(Deserialize)]
        struct W {
            a: WriteAction,
        }
    ...
    fn a_timing_action_accepts_the_map_form_and_the_tag_form() {
        // serde_yaml 0.9 rejects the single-key map form for an externally
        // tagged enum, which is the form anyone writing a part by hand uses.
        // Both must land on the same value — see `TimingAction`.
        #[derive(Debug, Deserialize)]
        struct T {
