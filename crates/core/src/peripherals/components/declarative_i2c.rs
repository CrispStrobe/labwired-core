...
pub struct GenericI2cDevice {
    ...
    /// **Tier 2**: the part's states, variables, FIFOs, timers and output pins
    /// (see [`RuleMachine`]). `None` ⇒ the descriptor declares none of it, and
    /// every rule code path below short-circuits — which is what keeps every
    /// Tier-1 device's transcript byte-identical.
...
impl GenericI2cDevice {
    ...
    /// Hand the rule machine the elapsed µs, firing any timer that came due.
    fn advance_rule_time(&mut self, us: u64) {
        ...
        };
        {
            ...
            machine.advance_time_us(us, &mut ctx);
        }
        self.rules = Some(machine);
    }
    ...
    fn write_inner(&mut self, data: u8) {
        ...
        // Pointerless (PCF8574-shaped): there is no address byte. Every byte is
        // data for the single register at 0, and a multi-byte write is a
        // sequence of updates to it — which is exactly what an expander does
        // when firmware streams port values without re-addressing.
    ...
    fn read(&mut self) -> u8 {
        ...
        if self.reg_auto_increment {
            ...
            if let Some((name, start)) = hit {
                ...
                self.raise(Event::Read { register: name }, 0);
            }
            return byte;
        }
        ...
        if !self.latched {
            ...
            if let Some(name) = name {
                ...
                // Tier 2: the datasheets clear on the read that LATCHES, so the
                // rule event fires at the same instant the built-in does.
                self.raise(Event::Read { register: name }, 0);
            }
            self.latched = true;
        }
