...
fn time_ns_is_the_end_of_the_step_like_the_ngspice_wrapper() {
    let mut adapter = AnalogCosimAdapter::from_netlist(RC, &rc_config()).expect("adapter builds");
...
fn a_probe_on_a_node_that_does_not_exist_is_refused() {
    ...
    let Err(err) = build_cosim_adapter_with_base(&manifest_model(config), Path::new(".")) else {
