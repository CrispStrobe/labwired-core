...
pub(crate) fn run_firmware(
    ...
    // Classic ESP32 (Xtensa LX6) fast-boot path. It now HAS a batched
    // orchestration — `run_firmware_esp32` hands off to
    // `run_firmware_xtensa_batched`, which drives `Machine::advance` at the
    // bus's declared safe tick interval, the same contract the ARM and RISC-V
    // paths use. The refusal that stood here was correct while that did not
    // exist: `--batched` is an ASSERTION about which loop ran, so accepting
    // the flag and quietly single-stepping would record a number for a path
    // that never executed. It is lifted only because the path is real, and
    // only for this chip.
