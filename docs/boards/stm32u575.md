...
| Validation         | real CubeU5 HAL firmware runs in-sim (`U575-HAL OK` + `BLINK n LD1=<0\|1>`); stock Zephyr `nucleo_u575zi_q` hello boots (`test_stm32u575_zephyr_survival`); Arduino matrix L0–L7 (CI lane); Zephyr matrix L0–L3 (local run; no CI workflow yet); unsupported-instruction audit 0/0/0 at 200k steps |
